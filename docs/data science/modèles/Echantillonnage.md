L'échantillonnage des données est une étape essentielle pour entrainer de façon efficace un modèle càd pour éviter le sur-apprentissage et obtenir le meilleur modèle (le plus généralisable).

Les données sont divisés en deux sous-ensembles qui servent soient :

* à l’apprentissage (train).
* à la validation (test).

Les principales méthodes d'échantillonnage sont : 

* validation non-croisée. Les données sont divisées en deux sous ensembles. Le modèle est construit sur l'échantillon d'apprentissage et validé sur celui de test.
...
* bootstrap ou ré-échantillonnage. A creuser.

__Induire__ inférence partir de cas particulier pour en déduire une généralité.

* bagging  (pour bootstrap aggregating)
* boosting ensemble d'algorithmes qui sur-pondérant les observations erronées différents algorithmes de base, tels que AdaBoost (Adaptive Boosting), Gradient Boostin.
